...
import { PhotoDailyPoint } from '@/hooks/bayan-open/useBayanOpenStats';
import { ChartStateWrapper } from './ChartStateWrapper';
import { fAxisTick, fDateLong, fNum } from '../lib/format';

interface PhotoGrowthChartProps {
  data: PhotoDailyPoint[];
  isLoading: boolean;
  error: string | null;
  onRetry: () => void;
}
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg px-3 py-2 shadow-soft text-xs">
      ...
      <p className="font-semibold">{fNum(payload[0].value)} photos uploaded</p>
    </div>
  );
};

export function PhotoGrowthChart({ data, isLoading, error, onRetry }: PhotoGrowthChartProps) {
  const periodTotal = data.reduce((sum, d) => sum + d.photos_added, 0);
      ...
      <CardHeader className="pb-2">
        ...
        {!isLoading && !error && data.length > 0 && (
          <p className="text-xs text-muted-foreground mt-0.5">
            <span className="text-secondary font-medium">+{fNum(periodTotal)}</span> Foto diunggah pada periode ini
          </p>
        )}
      </CardHeader>
      <CardContent>
        <ChartStateWrapper isLoading={isLoading} error={error} isEmpty={data.length === 0} onRetry={onRetry}>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="photos_added" fill="#2563eb" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </ChartStateWrapper>
      </CardContent>
